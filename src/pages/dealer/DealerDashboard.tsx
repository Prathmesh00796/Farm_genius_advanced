...
const DealerDashboard: React.FC = () => {
  ...
  const fetchStats = async () => {
    try {
      const [offersRes, inventoryRes, ordersRes] = await Promise.all([
        supabase.from('buy_offers').select('id', { count: 'exact' }).eq('dealer_id', user!.id).eq('status', 'active'),
        supabase.from('dealer_inventory').select('quantity_quintals').eq('dealer_id', user!.id),
        supabase.from('orders').select('status', { count: 'exact' }).eq('dealer_id', user!.id)
      ]);

      const totalInventory = inventoryRes.data?.reduce((sum, item) => sum + Number(item.quantity_quintals), 0) || 0;
      const pendingOrders = ordersRes.data?.filter(o => o.status !== 'delivered' && o.status !== 'cancelled').length || 0;
          ...
          <Card>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('dealer.activeOffers')}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('dealer.quintalsInStock')}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('dealer.pendingOrders')}</p>
              </div>
            </CardContent>
          </Card>
          <Card>
            <CardContent className="p-4 flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-sm text-muted-foreground">{t('dealer.completed')}</p>
              </div>
            </CardContent>
          </Card>
        </div>
